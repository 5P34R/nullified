...
app.config['APP_CONSUMER_SECRET'] = config["twitter"]["api_key_secret"]

# alternatively, add your key and secret to config.cfg
# config.cfg should look like:
APP_CONSUMER_KEY = config["twitter"]["api_key"]
APP_CONSUMER_SECRET = config["twitter"]["api_key_secret"]
...
def callback():
    ...
    print(f"eral reponse: {real_resp} real content: {real_content}")
